...
# 3. 공공 API 호출 함수 (최신 데이터 1000건 추출 후 내림차순 정렬)
@st.cache_data(ttl=3600) # 1시간마다 데이터 갱신
def fetch_disease_data():
    api_url_code = "Grid_20151204000000000316_1"
    base_url = f"http://211.237.50.150:7080/openapi/{API_KEY}/json/{api_url_code}"
    
    try:
        # 1차 호출: 전체 데이터 건수(totalCnt) 확인용 (1건만 요청)
        res_init = requests.get(f"{base_url}/1/1")
        res_init.raise_for_status()
        data_init = res_init.json()
        
        # totalCnt 파악
        if api_url_code in data_init and 'totalCnt' in data_init[api_url_code]:
            total_cnt = int(data_init[api_url_code]['totalCnt'])
        else:
            return pd.DataFrame()
            
        # 2차 호출: 가장 최신(마지막) 1000건 요청
        start_idx = max(1, total_cnt - 999) # 데이터가 1000건 미만일 경우 대비
        end_idx = total_cnt
        
        res_main = requests.get(f"{base_url}/{start_idx}/{end_idx}")
        res_main.raise_for_status()
        data_main = res_main.json()
        
        if api_url_code in data_main and 'row' in data_main[api_url_code]:
            df = pd.DataFrame(data_main[api_url_code]['row'])
            ...
            # 발생일자 기준 내림차순(최신순) 정렬 적용
            if '발생일자' in df.columns:
                df['발생일자'] = df['발생일자'].astype(str) # 정렬을 위해 문자열 변환
...
# 데이터 로딩 상태 표시
with st.spinner("농림축산검역본부 최신 실시간 데이터를 불러오는 중입니다..."):
...
if raw_df.empty:
    ...
    st.subheader("📋 실시간 전국 발생 상세 데이터 (최신순)")
